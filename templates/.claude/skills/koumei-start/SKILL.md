...
### 5.1. カスタムロールへの指示書の作成
TEAM.md の「チーム構成」テーブルに標準5ロール（koumei, analyst, ux-designer, tech-lead, devils-advocate）以外のロールが定義されている場合、それらのカスタムロールにも指示書を作成する。

1. `.agents/TEAM.md` のチーム構成テーブルを読み、標準ロール以外のエントリを特定する
2. 各カスタムロールの `.agents/{ロール名}/CLAUDE.md` を読み、責務を理解する
3. タスクの要件に関連するカスタムロールに対して指示書を作成する:
   - `.agents/{ロール名}/instructions/task-{番号}-instruction.md`
   - そのロールの責務に応じた指示内容を記載
4. タスク定義ファイルにもカスタムロールへの指示概要を含める
